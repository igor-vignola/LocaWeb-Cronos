...
description: Skill de polish e refinamento de UI do Cronos (filosofia Emil Kowalski). Use SEMPRE que decidir duração ou easing de animação, transform-origin de popovers, scale-on-press de botões, micro-interações, transições de estado, ou qualquer detalhe invisível que separa UI funcional de UI gostosa de usar. Regras-chave — cubic-bezier(0.23,1,0.32,1) pra ease-out, durações abaixo de 300ms em UI, scale(0.97) on active, nunca scale(0) na entrada, transform-origin na origem do trigger e não no center, CSS transitions preferíveis a keyframes em estados interrompíveis, prefers-reduced-motion respeitado. Pareada com design-taste-frontend no protótipo (Sprint 2) e nos templates Django (Sprint 3).
---
