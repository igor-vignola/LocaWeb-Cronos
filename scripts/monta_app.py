...
CSS_APP = r"""
/* troca de modo dentro da aba Hoje: sai um conteudo, entra outro, no lugar */
#modo-dia,#modo-tempo{transition:opacity .22s var(--e),transform .22s var(--e)}
.saindo{opacity:0;transform:translateY(-8px)}
.entrando{animation:entra .48s var(--e) both}
@keyframes entra{from{opacity:0;transform:translateY(14px)}}
.modo-h{display:flex;align-items:flex-start;gap:20px;margin:8px 0 18px}
.modo-h h2{font-size:24px;font-weight:650;letter-spacing:-.033em;margin-top:9px}
.modo-h p{font-size:13.5px;color:var(--tx2);margin-top:3px}
.modo-h .bt{margin-left:auto;flex-shrink:0;margin-top:6px}
/* o heroi vira botao */
...
.maq .play{background:var(--ac);border:0;color:#fff;width:38px;height:38px;border-radius:12px;
 cursor:pointer;display:grid;place-items:center;flex-shrink:0;transition:all .25s var(--e)}
.maq .play:hover{transform:translateY(-1px);box-shadow:0 8px 20px -8px rgba(37,99,235,.8)}
.maq .play:active{transform:scale(.95)}
.maq-d{flex-shrink:0;min-width:126px;color:#E8ECF3}
.maq-d b{font-size:16px;font-weight:650;display:block;font-variant-numeric:tabular-nums}
.maq-d span{font-size:11px;color:#8D97A8;font-variant-numeric:tabular-nums}
.maq-t{flex:1;min-width:0}
.lin{position:relative;height:42px;cursor:pointer}
.lin .base{position:absolute;inset:auto 0 16px 0;height:3px;border-radius:2px;
 background:rgba(255,255,255,.14)}
.lin .feito{position:absolute;inset:auto auto 16px 0;height:3px;border-radius:2px;
 background:var(--ac);box-shadow:0 0 10px rgba(37,99,235,.6)}
.lin .cab{position:absolute;bottom:11px;width:13px;height:13px;border-radius:50%;background:#fff;
 border:3px solid var(--ac);transform:translateX(-50%);transition:left .1s linear;
 box-shadow:0 2px 10px rgba(0,0,0,.6)}
.lin .falha{position:absolute;bottom:19px;width:2px;height:9px;border-radius:1px;
 background:rgba(240,72,62,.7);transform:translateX(-50%)}
.lin .mes-r{position:absolute;bottom:0;font-size:9.5px;color:#5F697C;letter-spacing:.1em;
 text-transform:uppercase;font-weight:600}
.vel{display:flex;gap:2px;background:rgba(255,255,255,.07);border-radius:9px;padding:3px;
 flex-shrink:0}
.vel button{background:0;border:0;font:inherit;font-size:11px;font-weight:650;color:#8D97A8;
 padding:6px 10px;border-radius:6px;cursor:pointer;transition:all .25s var(--e)}
.vel button.on{background:rgba(255,255,255,.14);color:#fff}
.maq-l{display:flex;gap:16px;margin-top:10px;flex-wrap:wrap;align-items:center}
.maq-l span{display:inline-flex;align-items:center;gap:6px;font-size:10.5px;color:#8D97A8}
.maq-l i{width:9px;height:9px;border-radius:2px}
.maq-l .l-ok{background:var(--ac)}.maq-l .l-no{background:rgba(240,72,62,.8)}
.maq-l .dica{margin-left:auto;color:#5F697C}
/* os cards que reagem */
.maq-c.pisca{animation:pisca .55s var(--e)}
@keyframes pisca{0%{box-shadow:0 0 0 0 rgba(37,99,235,.28),var(--s1)}
 100%{box-shadow:0 0 0 9px rgba(37,99,235,0),var(--s1)}}
.big-n{display:flex;align-items:baseline;gap:2px;margin:2px 0 10px}
.big-n b{font-size:52px;font-weight:700;letter-spacing:-.05em;line-height:1;
 font-variant-numeric:tabular-nums}
.big-n u{font-size:20px;font-weight:600;color:var(--tx2);text-decoration:none}
.bar{height:8px;border-radius:4px;background:#EDEFF3;overflow:hidden}
.bar i{display:block;height:100%;border-radius:4px;background:var(--ac);
 transition:width .5s var(--e),background .5s var(--e)}
.bar.sm{height:6px}
.par{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:2px}
.par div em{font-size:10px;font-style:normal;font-weight:700;letter-spacing:.1em;
 text-transform:uppercase;color:var(--tx3);display:block}
.par div b{font-size:32px;font-weight:700;letter-spacing:-.045em;display:block;line-height:1;
 margin-top:4px;font-variant-numeric:tabular-nums}
.par div span{font-size:11px;color:var(--tx2);display:block;margin-top:4px}
.ver-l{margin-top:14px}
.ver{display:inline-block;font-size:11px;font-weight:700;letter-spacing:.07em;
 text-transform:uppercase;padding:5px 11px;border-radius:8px}
.ver.ok{background:var(--okl);color:var(--ok)}.ver.no{background:var(--nol);color:var(--no)}
/* lados do erro */
.lados{display:grid;grid-template-columns:1fr 1fr;gap:16px}
.lado em{font-size:10px;font-style:normal;font-weight:700;letter-spacing:.1em;
 text-transform:uppercase;color:#8D97A8;display:block}
.lado b{font-size:44px;font-weight:700;letter-spacing:-.05em;display:block;line-height:1;
 margin-top:6px;color:#FF7D74}
.lado b.zero{color:#37D39B}
.lado span{font-size:11.5px;color:#8D97A8;display:block;margin-top:6px;line-height:1.45}
.lado.sep{border-left:1px solid rgba(255,255,255,.1);padding-left:16px}
/* meses e semana */
.mes-l{display:flex;flex-direction:column;gap:13px}
.mes-h{display:flex;align-items:baseline;gap:9px;margin-bottom:6px}
.mes-h b{font-size:13.5px;font-weight:650;text-transform:capitalize}
.mes-h span{margin-left:auto;font-size:15px;font-weight:700;
 font-variant-numeric:tabular-nums}
.mes-h .ok{color:var(--ok)}.mes-h .wn{color:var(--wn)}.mes-h .no{color:var(--no)}
.mes-f{font-size:11px;color:var(--tx2);margin-top:5px}
.mes-f b{color:var(--hd);font-weight:650}
.sem-l{display:flex;flex-direction:column;gap:9px}
.sem{display:flex;align-items:center;gap:11px}
.sem-d{width:32px;font-size:12px;font-weight:600;color:var(--tx)}
.sem .bar{flex:1}
.sem-v{width:38px;text-align:right;font-size:12.5px;font-weight:650;
 font-variant-numeric:tabular-nums}
.sem-q{width:38px;text-align:right;font-size:11px;color:var(--tx3);
 font-variant-numeric:tabular-nums}
@media (max-width:900px){.maq-p{flex-wrap:wrap}.maq-t{order:9;width:100%}
 .lados,.par{grid-template-columns:1fr}.lado.sep{border:0;padding:0}}
/* marca d'agua: o icone do cabecalho ampliado, sangrando pelo canto do card */
.dagua{position:absolute;top:-52px;right:-46px;color:var(--ink);opacity:.032;
 pointer-events:none;z-index:0;line-height:0}
.dagua svg{stroke-width:.9}
.cd.dk .dagua{color:#fff;opacity:.05}
.cd>.hdr,.cd>.nt,.cd>div,.cd>table,.cd>p{position:relative;z-index:1}
